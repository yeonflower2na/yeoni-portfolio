...
export default function AxPostView({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const listRes = await fetch('/ax/posts.json')
        if (!listRes.ok) throw new Error(`posts.json ${listRes.status}`)
        const posts: AxPost[] = await listRes.json()
      ...
      <main className="ax-detail">
        ...
        {status === 'notfound' && (
          <div className="ax-detail-state">
            <p>글을 찾을 수 없습니다.</p>
            <Link href="/ax" className="ax-back" prefetch={false}>
